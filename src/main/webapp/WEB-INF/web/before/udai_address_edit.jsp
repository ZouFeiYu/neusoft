				...
				<div class="user-content__box clearfix bgf">
					...
					<form class="user-addr__form form-horizontal" role="form"onsubmit="return false">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input class="form-control" id="name" placeholder="请输入姓名" type="text" value="${address.recName }">
							</div>
						</div>
						<div class="form-group">
							<label for="details" class="col-sm-2 control-label">收货地址：</label>
							<div class="col-sm-10">
								<input class="form-control" id="details" value="${address.detail}" placeholder="建议您如实填写详细收货地址，例如街道名称，门牌号码等信息" maxlength="30" type="text">
							</div>
						</div>
						<div class="form-group">
							<label for="phone" class="col-sm-2 control-label">手机号码：</label>
							<div class="col-sm-6">
								<input class="form-control" id="mobile" value="${address.phone }" placeholder="请输入手机号码" type="text">
							</div>
						</div>
						<div class="form-group">
							<label for="phone" class="col-sm-2 control-label">电话号码：</label>
							<div class="col-sm-6">
								<input class="form-control" id="telphone" value="${address.telphone}" placeholder="请输入电话号码" type="text">
							</div>
						</div>
							...
							<div class="col-sm-offset-2 col-sm-6">
								<div class="checkbox">
									...
									<c:if test="${address.recDefault==1}">
										<input type="checkbox" id="setDefault" checked="checked"><i></i> 设为默认收货地址
									</c:if>
									<c:if test="${address.recDefault==0}">
										<input type="checkbox" id="setDefault" ><i></i> 设为默认收货地址
									</c:if>
									</label>
								</div>
							</div>
		...
		$("#save-sumbit").click(function(){
			...
			if(recName==""||recName==null){
				...
				alert("联系方式不能为空");
					...
					$.ajax({
						url:"${pageContext.request.contextPath}/web/main/updateAddress/"+userId,
						data:"userId="+userId+"&id="+addressId+"&recName="+recName+"&detail="+detail+"&phone="+phone+"&telphone="+telphone+"&recDefault="+recDefault,
						type:"post",
						dataType:"json",
